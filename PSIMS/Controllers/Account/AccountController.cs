   ...
   // [Authorize]
    public class AccountController : Controller
    {
            ...
            try
            {
                ...
                switch (result)
                {
                    case SignInStatus.Success:
                        ...
                        {
                            _menus.AddRange((from p in db.MenuModels.AsEnumerable()
                                             join q in db.MenuUserRoleModels on p.MenuId equals q.MenuId
                                             where (q.RoleId == role.Id)
                        ...
                        FormsAuthentication.SetAuthCookie(user.UserName, false); // set the formauthentication cookie
        ...
        public JsonResult CheckExpiry()
        {
            using(ApplicationDbContext db = new ApplicationDbContext())
            {
               ...
               /* var notif = (from n in db.Notifications
                ...
                var list = db.Stocks.SqlQuery("select * from dbo.Stock where GETDATE() > ExpiryDate").ToList();
        ...
        public JsonResult CheckStockLevel()
        {
            using(ApplicationDbContext db = new ApplicationDbContext())
            {
                var list = db.Stocks.SqlQuery("select * from dbo.Stock s, dbItem i where i.ID = s.ItemID and s.Qty<i.AlertQty and s.Qty>0;").ToList();
                return Json(list.Count(), JsonRequestBehavior.AllowGet);
            }
